# Технический проект: LLM Telegram Bot
...
- **Тестирование:** pytest
- **Управление зависимостями:** uv + requirements.txt
- **Деплой:** Docker
...
│   ├── product_idea.md
│   └── technical_design.md
├── requirements.txt       # Зависимости: aiogram, openai, python-dotenv, pytest
...
**⚙️ Публичные настройки (config.py)**
```python
# Настройки LLM
...
- **Безопасность**: секреты в .env, публичные настройки в коде
...
2. Создание `.env` с тестовыми токенами
3. Заполнение `src/system_prompt.md`
4. Локальный запуск: `make run`
5. Тестирование через Telegram

**🌐 Деплой на VPS**
1. Подключение к VPS по SSH
2. Клонирование репозитория: `git clone ...`
3. Создание продуктивного `.env`
4. Настройка `system_prompt.md` для продакшена
5. Запуск: `make run`
...
Технический проект **LLM Telegram Bot** полностью проработан и готов к реализации. Все ключевые решения приняты в соответствии с принципом **KISS (Keep It Simple, Stupid)**, что обеспечивает:
...
2. **Отсутствие БД** - вся логика в памяти для MVP
3. **Контейнеризация** - простой деплой на любом VPS
4. **Двойное логирование** - удобство разработки и мониторинга
5. **Гибридная конфигурация** - безопасность + простота

### Следующие шаги

1. **Создание репозитория** и базовой структуры файлов
2. **Настройка окружения** с uv и requirements.txt
3. **Разработка core функций** по модулям (config → llm_client → handlers → main)
4. **Создание system_prompt.md** с информацией о вашей компании
5. **Локальное тестирование** через тестового бота
6. **Деплой на VPS** и финальное тестирование
